...
import { Wallet, Calculator, Star, Clock, ShieldCheck, ChevronRight, History, MessageSquare, Settings, Bell, TrendingUp, UserCheck, ShieldAlert, FileText, BarChart3 } from 'lucide-react';
...
const AccountantDashboard: React.FC<AccountantDashboardProps> = ({ onNavigate, accountant }) => {
  const [balance] = useState(320000);
    ...
    <div className="p-6 space-y-6 animate-in fade-in duration-500 pb-24 h-full overflow-y-auto bg-slate-50/30">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 tracking-tighter">Cabinet Expertise</h2>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{accountant.firm}</p>
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="mt-6 w-full pt-6 border-t border-white/10 flex justify-around">
             <div className="text-center">
                <p className="text-[8px] font-black text-slate-400 uppercase">En Attente</p>
                <p className="text-lg font-black text-amber-400">{pendingBalance.toLocaleString()} F</p>
             </div>
             <div className="w-px h-8 bg-white/10"></div>
             <div className="text-center">
                <p className="text-[8px] font-black text-slate-400 uppercase">Bilans en cours</p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-2">
           ...
           <p className="text-[10px] font-black text-slate-400 uppercase">Déclarations</p>
        ...
        <div className="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-2">
           ...
           <p className="text-[10px] font-black text-slate-400 uppercase">Audits</p>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-3">
          {[
            { id: 1, client: 'Boutique Kiki', type: 'TVA Mensuelle', price: 15000, status: 'PAID', time: 'Il y a 3h' },
            { id: 2, client: 'Ets. Mba & Fils', type: 'Bilan Annuel', price: 150000, status: 'PAID', time: 'Hier' },
            { id: 3, client: 'Restaurant Le Phare', type: 'Audit Social', price: 45000, status: 'PENDING', time: '3 jours' },
          ].map(item => (
      ...
      <div className="bg-slate-100 p-6 rounded-[2.5rem] border border-slate-200 flex items-start gap-3">
         ...
         <p className="text-[10px] text-slate-600 font-bold leading-relaxed">
           Votre cabinet est certifié Expertise Maraude. Les honoraires sont versés sur votre portefeuille après validation de la mission par le client.
         </p>
      </div>
    </div>
  );
