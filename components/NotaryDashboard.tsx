...
import { Wallet, Scale, Star, Clock, ShieldCheck, ChevronRight, History, MessageSquare, Settings, Bell, TrendingUp, UserCheck, ShieldAlert, FileText, Gavel } from 'lucide-react';
...
const NotaryDashboard: React.FC<NotaryDashboardProps> = ({ onNavigate, notary }) => {
  const [balance] = useState(450000);
  const [pendingBalance] = useState(120000);
    ...
    <div className="p-6 space-y-6 animate-in fade-in duration-500 pb-24 h-full overflow-y-auto bg-slate-50/30">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 tracking-tighter">Étude Notariale</h2>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{notary.office}</p>
          </div>
        </div>
      ...
      <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="mt-6 w-full pt-6 border-t border-white/10 flex justify-around">
             ...
             <div className="text-center">
                <p className="text-[8px] font-black text-slate-400 uppercase">Actes en cours</p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-2">
           ...
           <p className="text-[10px] font-black text-slate-400 uppercase">Authentifications</p>
        ...
        <div className="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-2">
           ...
           <p className="text-[10px] font-black text-slate-400 uppercase">Successions</p>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-3">
          {[
            { id: 1, client: 'SCI Delta', type: 'Vente Immobilière', price: 250000, status: 'PAID', time: 'Il y a 1h' },
            { id: 2, client: 'M. Jean-Pierre', type: 'Testament', price: 75000, status: 'PAID', time: 'Hier' },
            { id: 3, client: 'SARL Gabon Log', type: 'Statuts', price: 125000, status: 'PENDING', time: '2 jours' },
          ].map(item => (
